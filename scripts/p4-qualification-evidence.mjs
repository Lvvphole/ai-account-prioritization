...
const parseInvocationAudit = (path) => {
  ...
  const models = [...new Map(
    started.map((row) => [`${row.provider}:${row.model ?? ""}`, {
...
export function buildEvidenceManifest({
  ...
  if (
    outcome === "success" &&
    (!report.present || !admissionPresent || invocations.startedCount === 0 ||
  ...
  const admission = artifactDescriptor(
    sourceDir,
    admissionPath,
  ...
  const manifest = {
    ...
    qualification: {
      ...
      failureReasonCode: outcome === "success" ? null : (effectiveFailureCode ?? "UNCLASSIFIED_COMMAND_FAILURE"),
...
const assertArtifact = (artifact, name) => {
  ...
  if (typeof artifact.present !== "boolean") throw new Error(`${name}.present must be boolean.`);
  if (artifact.present) {
...
export function validateEvidenceManifest(manifest) {
  if (!manifest || typeof manifest !== "object") throw new Error("Evidence manifest must be an object.");
  ...
  const runAttempt = positiveInteger(manifest.workflow?.producerRunAttempt, "workflow.producerRunAttempt");
  if (!SHA_PATTERN.test(nonEmpty(manifest.workflow?.qualificationSourceSha, "workflow.qualificationSourceSha"))) {
    throw new Error("workflow.qualificationSourceSha is invalid.");
  }
  ...
  if (!SHA256_PATTERN.test(nonEmpty(qualification.policyFileSha256, "qualification.policyFileSha256"))) {
    throw new Error("qualification.policyFileSha256 is invalid.");
  }
  if (qualification.outcome === "success") {
    if (qualification.exitCode !== 0 || qualification.failureReasonCode !== null ||
    ...
    if (qualification.failureMessageSha256 !== null &&
  ...
  if (!invocations || typeof invocations.path !== "string" ||
    !invocations.path.startsWith("p4-output/")) {
    throw new Error("invocations.path is invalid.");
  }
  for (const [key, value] of [["startedCount", invocations.startedCount], ["completedCount", invocations.completedCount]]) {
    if (!Number.isSafeInteger(value) || value < 0) throw new Error(`invocations.${key} is invalid.`);
  }
  if (typeof invocations.present !== "boolean") throw new Error("invocations.present must be boolean.");
  ...
  } else if (invocations.sha256 !== null || invocations.startedCount !== 0 || invocations.completedCount !== 0) {
  ...
  for (const model of invocations.models) {
    if (!PROVIDERS.has(model.provider)) throw new Error("invocations.models provider is invalid.");
  ...
  const expectedTag = `p4-qualification-${runId}-${runAttempt}`;
  if (manifest.release?.tag !== expectedTag) throw new Error("release.tag is not bound to producer identity.");
  const transferArtifact = nonEmpty(manifest.release?.transferArtifact, "release.transferArtifact");
  if (transferArtifact !== `p4-qualification-transfer-${runId}-${runAttempt}`) {
    throw new Error("release.transferArtifact is not bound to producer identity.");
  }

  if (qualification.outcome === "success") {
    if (!manifest.artifacts.report.present || !manifest.artifacts.admission.present ||
      !manifest.artifacts.admission.publishEligible || invocations.startedCount === 0 ||
      invocations.startedCount !== invocations.completedCount) {
...
const runQualification = async ({ sourceDir }) => {
  ...
  const sourceSha = nonEmpty(process.env.P4_QUALIFICATION_SOURCE_SHA, "P4_QUALIFICATION_SOURCE_SHA");
  ...
  const auditPreloadPath = join(dirname(fileURLToPath(import.meta.url)), "p4-provider-invocation-audit.cjs");
  ...
  const childEnv = {
    ...
    NODE_OPTIONS: [process.env.NODE_OPTIONS, `--require=${auditPreloadPath}`].filter(Boolean).join(" "),
  };
  ...
  try {
    ...
    const fallback = {
      contractVersion: P4_EVIDENCE_CONTRACT_VERSION,
      workflow: { runId, producerRunAttempt: runAttempt, qualificationSourceSha: sourceSha },
      decision: {
      ...
      qualification: {
        ...
        policyFileSha256: sha256File(join(absoluteSourceDir, "config/p4-qualification-policy.json")),
      },
      invocations: {
...
const parseArgs = (argv) => {
  if (argv[0] !== "run") throw new Error("Usage: p4-qualification-evidence.mjs run --source-dir <path>");
